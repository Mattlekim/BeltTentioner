...
namespace BeltTensionTest.WPF.ViewModels
{
    public class TestingViewModel : ViewModelBase, IDisposable
    {
        ...
        // Throttle graph redraws generated by frequent live updates
        private DateTime _lastGraphRedraw = DateTime.MinValue;
        private DateTime _lastMotorRedraw = DateTime.MinValue;
        private static readonly TimeSpan _minGraphInterval  = TimeSpan.FromMilliseconds(50);
        private static readonly TimeSpan _minMotorInterval  = TimeSpan.FromMilliseconds(50);
        ...
        public void UpdateLivePreview(float surge, float sway, float heave)
        {
            ...
            // Throttle redraws coming from high-frequency telemetry to avoid UI thread overload
            var now = DateTime.UtcNow;
            if (now - _lastGraphRedraw >= _minGraphInterval)
            {
                _lastGraphRedraw = now;
        ...
        public void UpdateMotorOutput(float left, float right, Rotation rotation)
        {
            ...
            // Throttle motor graph redraws to avoid overwhelming UI thread
            var now = DateTime.UtcNow;
            if (now - _lastMotorRedraw >= _minMotorInterval)
            {
                _lastMotorRedraw = now;
